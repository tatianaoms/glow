...
<div class="nuevos-tips">
    @forelse($posts as $post)
        <div class="tip">
            ...
            <small class="fecha-post">Publicado el: {{ $post->created_at->format('d/m/Y') }}</small>

            @auth
                <div class="acciones">
                    <a href="{{ route('posts.edit', $post->id) }}" class="boton-editar">Editar</a>
                    <form action="{{ route('posts.destroy', $post->id) }}" method="POST" style="display:inline;"
                        onsubmit="return confirm('¿Está seguro que desea eliminar este Tip?');">
                        @csrf
                        @method('DELETE')
    ...
    @empty
        <p class="mensaje-vacio">Todavía no hay tips de cabello publicados. <br> ¡Muy pronto!</p>
    @endforelse
</div>
